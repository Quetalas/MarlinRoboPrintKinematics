...
void report_current_position();
 ...
 // Don't collect anchor positions in array because there are no A_x, D_x or D_y
  ...
  #define HANGPRINTER_IK(V) do {                             \
    ...
    if(V[Z_AXIS] < 300.0) { \
      z = 300.0;  \
    } else if(V[Z_AXIS] > 350.0) {  \
      z = 350.0;  \
    } \
    line_lengths[A_AXIS] = -1.0*(V[X_AXIS] + 650.0 - sqrt(sq(405.0) - sq(110.0 + V[Y_AXIS]) - sq(z))); \
    line_lengths[B_AXIS] = 1.0*(-V[X_AXIS] + 650.0 - sqrt(sq(405.0) - sq(110.0 + V[Y_AXIS]) - sq(z))); \
    line_lengths[C_AXIS] = 1.0*(-V[X_AXIS] + 650.0 - sqrt(sq(405.0) - sq(110.0 - V[Y_AXIS]) - sq(z))); \
    line_lengths[D_AXIS] = 1.0*(V[X_AXIS] + 650.0 - sqrt(sq(405.0) - sq(110.0 - V[Y_AXIS]) - sq(z))); \
  }while(0)
